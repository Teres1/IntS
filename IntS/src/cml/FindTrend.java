...
public class FindTrend {
    ...
    private List<Double> findTrendSecondWay(double []arr){
        ...
        secondIndexOfLinearRegresion=(arrElementMultiplyByIndexSum-indexSum*arrSum/arr.length)
                /(indexSquareSum-indexSum*indexSum/arr.length);
        firstIndexOfLinearRegresion=(arrSum-secondIndexOfLinearRegresion*indexSum)/arr.length;
        for (int i = 0; i < arr.length; i++) {
            result.add(arr[i]);
        }
        for (int i = 0; i < 5; i++) {
            result.add(firstIndexOfLinearRegresion
                    +result.get(result.size()-2)*secondIndexOfLinearRegresion);
        }
        return result;
    }
